    ...
    def parse_file(self) -> None:
        ...
        with open(self.file_path, 'r', encoding='utf-8') as file:
            ...
            for block in question_blocks:
                ...
                options_pattern = r'\*\s*\(([a-d])\)\s*([^*]+)'
                options_matches = re.findall(options_pattern, content[content.find(question_text):])

                # Extract only the first 4 options
                options = [(letter, text.strip()) for letter, text in options_matches[:4]]
